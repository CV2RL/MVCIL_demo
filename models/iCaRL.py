...
from convs.myNetwork import network, point_network
...
class iCaRLmodel:
    ...
    def __init__(self, dataset_name,init_class, feature_extractor, batch_size, increment, memory_size, epochs, learning_rate,
        ...
        if "modelnet" in dataset_name:
            self.model = point_network(init_class, feature_extractor)
        else:
            self.model = network(init_class, feature_extractor)
        self.exemplar_set = []
        self.class_mean_set = []
    ...
    def beforeTrain(self):
        ...
        if self.init_class > self.increment:
            self.model.Incremental_learning(self.init_class)
        self.model.train()
    ...
    def _compute_loss(self, indexs, imgs, target):
        ...
        target = get_one_hot(target, self.init_class)
    ...
    def afterTrain(self, accuracy):
        self.model.eval()
        m = int(self.memory_size / self.init_class)
        self._reduce_exemplar_sets(m)
        for i in range(self.init_class - self.increment, self.init_class):
            print('construct class %s examplar:' % (i), end='')
            images = self.train_dataset.get_image_class(i)
            self._construct_exemplar_set(images, m)
        self.init_class += self.increment
